...
// Here follows a working example of how to use DynamicQueue. 
...
int main() {
   ...
   // Make instance of DynamicQueue containing objects of type int:
   DynamicQueue<int> queue;

   // Reserve a reasonable size for the queue if the maximum number of 
   ...
   // Output all entries without removing from queue
   for (i = 0; i < queue.GetNum(); i++) {
      printf("\n%2i:  %5i", i, queue[i]);
   }
   ...
   // Remove entries, First-In-First-Out
   while (queue.GetNum()) {
      printf("\n     %5i", queue.Get());
   }
